const fullProjectsData = [
    {
        name: "Business Bankruptcy Prediction",
        description: "Developed a deep learning model to predict bankruptcies of businesses using CNN and LSTM networks, acheiving 96.11% accuracy.",
        stack: ["Python", "scikit-learn"],
        status: "Completed",
        githubLink: "https://github.com/Harsha107/Bankruptcy-prediction"
    },
    {
        name: "Amazon Product Price Watcher & Notification System",
        description: "Implemented a web scraper and built a web app using Streamlit to monitor and track the price changes of the entered product. It then sends an email notification if the price is dropped below the set threshold",
        stack: ["Python", "Streamlit", "BeautifulSoup"],
        status: "Pending",
        githubLink: "https://github.com/Harsha107/Web-scraping"
    },
    {
        name: "Engery Consumption Prediction",
        description: "Built an explainable AI model for predicting energy consumption in Ontario, Canada.",
        stack: ["Python", "SHAP", "QLattice"],
        status: "Completed",
        githubLink: "https://github.com/Harsha107/Electricity-Consumption-Prediction"
    },
    {
        name: "",
        description: "",
        stack: {},
        status: "",
    },
    {
        name: "",
        description: "",
        stack: {},
        status: "",
    },
    {
        name: "",
        description: "",
        stack: {},
        status: "",
    },
    {
        name: "",
        description: "",
        stack: {},
        status: "",
    },
    {
        name: "",
        description: "",
        stack: {},
        status: "",
    },
];

export default fullProjectsData;